...
function renderRanking() {
  ...
  const rows = computeRanking(results);
  const rankRows = rows
    .map((r) => `<tr>
      <td>${r.rank}</td>
  ...
  <section class="card no-print">
    <details data-uikey="ranking" ${open ? 'open' : ''}>
      <summary class="secsum"><b>🏆 앵그리랭킹</b> <span class="hint-inline">— 앵그리대회 누적 (승수 우선 · 득실차 순)</span></summary>
      ${rows.length
        ? `<table class="detail-table" style="max-width:460px;margin-top:8px">
