...
function renderBracketCanvas(b, date) {
  const isReg = b.type === 'regular';
  const maxCourts = Math.max(...b.rounds.map((rd) => rd.games.length));
  const name = (id) => viewerName(b, id);
...
async function saveBracketImage() {
  ...
  let canvas;
  try {
    canvas = renderBracketCanvas(p.b, p.date);
  } catch (e) {
    toast('이미지를 만들 수 없습니다: ' + e.message);
    return;
  }
  const fname = `앵그리대진표${p.date ? '_' + p.date.replace(/[^0-9]/g, '') : ''}.png`;
  canvas.toBlob(async (blob) => {
    if (!blob) { toast('이미지 생성에 실패했습니다.'); return; }
    const file = new File([blob], fname, { type: 'image/png' });
    // 모바일: 공유 시트(사진 저장) 우선
    if (navigator.canShare && navigator.canShare({ files: [file] })) {
      try { await navigator.share({ files: [file] }); return; } catch (e) { if (e && e.name === 'AbortError') return; /* 실패 시 다운로드로 폴백 */ }
    }
    // PC/그 외: 다운로드
